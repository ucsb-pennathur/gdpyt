...
# generate sheared checkerboard
"""
tform1 = AffineTransform(scale=(1.3, 1.1), rotation=0, shear=0.0, translation=(110, 30))
tform2 = AffineTransform(scale=(1.3/1.1, 1.1/1.1), rotation=3, shear=0.1, translation=(170, 120))
tform3 = AffineTransform(scale=(1.3/1.25, 1.1/1.25), rotation=-3, shear=0.2, translation=(190, 160))
image1 = warp(data.checkerboard()[:98, :98], tform1.inverse, output_shape=(200, 310))
image2 = warp(data.checkerboard()[:98, :98], tform2.inverse, output_shape=(200, 310))
image3 = warp(data.checkerboard()[:98, :98], tform3.inverse, output_shape=(200, 310))
"""

# plot initial images
"""
fig, [ax1, ax2, ax3] = plt.subplots(nrows=3, figsize=(5, 10))
ax1.imshow(image1)
ax1.axis('off')
ax2.imshow(image2)
ax2.axis('off')
...
# save synthetic chessboard images
"""
savepath = '/Users/mackenzie/Desktop/calibrateCamera/data_synthetic'
synthetic_images = [image1, image2, image3]
for ind, img in enumerate(synthetic_images):
    img = util.invert(img)
    img = rescale_intensity(img, out_range=np.uint8)
    io.imsave(savepath+'/synthetic_chessboard_{}.tif'.format(ind), img, check_contrast=False)
"""
...
DATA_DIR = "/Users/mackenzie/Desktop/calibrateCamera/data_synthetic/"
