    ...
    def unpack(self):
        ...
        if self.dataset == 'synthetic_overlap_noise-level':
            ...
            if self.particle_distribution == 'grid-random-z' and SINGLE_PARTICLE_CALIBRATION is False:
                ...
                base_dir = join(base_dir, 'random', 'particle_density_' + self.particle_density)
                calib_cropping_specs = {'xmin': 220, 'xmax': 580, 'ymin': 670, 'ymax': 1024, 'pad': 10}  # {'xmin': 623, 'xmax': 698, 'ymin': 186, 'ymax': 261, 'pad': 30}
            ...
            if self.collection_type == 'calibration':
                ...
                if self.sweep_param is not None:
                    if self.single_particle_calibration is True:
                        ...
                        CALIB_RESULTS_PATH = join(base_dir, 'results/calibration-spct-{}-{}'.format(self.sweep_method, self.sweep_param))
                    else:
                        CALIB_ID = calib_id + 'idpt_{}'.format(self.sweep_param)
                        CALIB_RESULTS_PATH = join(base_dir, 'results/calibration-idpt-{}-{}'.format(self.sweep_method, self.sweep_param))
                else:
                    CALIB_ID = calib_id
                    if self.single_particle_calibration is True:
            ...
            if self.collection_type == 'test':
                ...
                if self.sweep_param is not None:
                    if self.single_particle_calibration is True:
                        ...
                        TEST_RESULTS_PATH = join(base_dir, 'results/test-idpt-{}-{}'.format(self.sweep_method, self.sweep_param))
                else:
        ...
        if self.dataset == '10X_1Xmag_2.15umNR_HighInt_0.12XHg':
            ...
            image_subset = [1, 151, calibration_z_step_size]
            threshold_modifier = optics.bkg_mean + optics.bkg_noise * 5
            ...
            if self.collection_type == 'calibration':
                ...
                # test dataset information
                TEST_SUBSET = [1, 151, calibration_z_step_size]
                TRUE_NUM_PARTICLES_PER_IMAGE = 1
                IF_TEST_IMAGE_STACK = 'first'
                TAKE_TEST_IMAGE_SUBSET_MEAN = []
                TEST_PARTICLE_ID_IMAGE = 'calib_75.tif'
        ...
        if self.dataset == '20X_1Xmag_0.87umNR':
            ...
            if self.collection_type == 'calibration':
                ...
                TEST_BASE_STRING = 'test_'  # 'nilered870nm_'
                TEST_GROUND_TRUTH_PATH = None
                ...
                TEST_PARTICLE_ID_IMAGE = 'test_163.tif'  # baseline_image
        ...
        if self.dataset == '20X_1Xmag_2.15umNR_HighInt_0.03XHg':
            ...
            if self.particle_distribution == 'glass':
                ...
                if self.collection_type == 'meta-test':
                    ...
                    metaset_i, metaset_f = 0, 3  # subset_f, subset_f + 1
                    XY_DISPLACEMENT = [[0, 0]]
            ...
            if self.collection_type == 'calibration':
                ...
                # calib dataset information
                CALIB_SUBSET = None  # [10, 70, 3]
                CALIBRATION_Z_STEP_SIZE = 1.0
                TRUE_NUM_PARTICLES_PER_CALIB_IMAGE = 1
                BASELINE_IMAGE = 'calib_35.tif'  # 'calib_30.tif'
                ...
                if 'none' in list(CALIB_PROCESSING.keys()):
                    CALIB_THRESHOLD_MODIFIER = 0.225  # optics.bkg_mean + optics.bkg_noise * 3
                else:
                    CALIB_THRESHOLD_MODIFIER = optics.bkg_mean + optics.bkg_noise * 3
                CALIB_THRESHOLD_PARAMS = {CALIB_THRESHOLD_METHOD: [CALIB_THRESHOLD_MODIFIER]}
                ...
                # {TEST_PROCESSING_METHOD: {'args': [square(TEST_PROCESSING_FILTER_SIZE), None, 'wrap']}}
                TEST_THRESHOLD_METHOD = 'manual_percent'
                TEST_THRESHOLD_MODIFIER = 0.225  # optics.bkg_mean + optics.bkg_noise * 3
                TEST_THRESHOLD_PARAMS = {TEST_THRESHOLD_METHOD: [TEST_THRESHOLD_MODIFIER]}
        ...
        if self.dataset == '11.06.21_z-micrometer-v2':
            ...
            if self.collection_type == 'meta-test':
                ...
                IF_TEST_IMAGE_STACK = 'first'  # 'first'
                TAKE_TEST_IMAGE_SUBSET_MEAN = [0, 1]  # [metaset_i, metaset_f]
                TEST_PARTICLE_ID_IMAGE = calib_baseline_image  # was calib_30.tif
                TEST_TEMPLATE_PADDING = 1  # SPCT: 1, IDPT: 16
        ...
        if self.dataset == '11.02.21-BPE_Pressure_Deflection_20X':
            ...
            if self.collection_type == 'calibration':
                ...
                CALIB_RESULTS_PATH = join(base_dir, 'results/cal-{}_test-{}'.format(self.sweep_param[0], self.sweep_param[1]))
                if not os.path.exists(CALIB_RESULTS_PATH):
                    os.makedirs(CALIB_RESULTS_PATH)
            ...
            if self.collection_type == 'test':
                ...
                TEST_RESULTS_PATH = join(base_dir, 'results/test-{}_cal-{}'.format(self.sweep_param[1], self.sweep_param[0]))
                if not os.path.exists(TEST_RESULTS_PATH):
                    os.makedirs(TEST_RESULTS_PATH)
