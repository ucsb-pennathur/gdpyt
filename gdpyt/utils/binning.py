...
def bin_local_rmse_z(df, column_to_bin='true_z', bins=20, min_cm=0.9, z_range=None, round_to_decimal=0,
    ...
    if z_range is not None:
        dfc = dfc[(dfc['true_z'] > z_range[0]) & (dfc['true_z'] < z_range[1])]
...
def bin_local(df, column_to_bin='true_z', bins=20, min_cm=None, z_range=None, round_to_decimal=0,
              true_num_particles=None):
    """
    ...
    # define extents of z-range
    if z_range is not None:
        dfb = dfb[(dfb['true_z'] > z_range[0]) & (dfb['true_z'] < z_range[1])]

    # if c_m is below minimum c_m, change 'z' to NaN:
    if min_cm is not None:
        dfb['z'] = np.where((dfb['cm'] < min_cm), np.nan, dfb['z'])

    # returns an identical dataframe but adds a column named "bin"
    if (isinstance(bins, int) or isinstance(bins, float)):
        dfb = bin_by_column(dfb, column_to_bin=column_to_bin, number_of_bins=bins, round_to_decimal=3)
    elif isinstance(bins, (list, tuple, np.ndarray)):
        dfb = bin_by_list(dfb, column_to_bin=column_to_bin, bins=bins, round_to_decimal=round_to_decimal)
    ...
    if 'cm' in dfb.columns:
        ...
        if true_num_particles is not None:
            ...
            dfb = dfb.join(dfc[['num_bind', 'num_meas', 'percent_meas', 'true_num_particles', 'true_percent_meas']])
        else:
