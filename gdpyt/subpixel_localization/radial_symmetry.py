"""
Radial symmetry method for subpixel localization of particles - TRAIT2D
author: C. Eggeling, et al.
"""
...
class Detectors(object):
    ...
    def radialsym_centre(self, img):
        ...
        w = np.divide(dImag2, np.sqrt(
            np.multiply((xm - xcentroid), (xm - xcentroid)) + np.multiply((ym - ycentroid), (ym - ycentroid))))
    ...
    def detect(self, frame):
        ...
        # find local maximum
        peaks_coor = peak_local_max(self.img_sef, min_distance=self.min_distance,
                                    threshold_rel=self.threshold_rel)  # min distance between peaks and threshold_rel - min value of the peak - in relation to the max value
