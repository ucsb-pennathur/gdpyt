...
SAME_ID_THRESH = 15  # maximum distance=sqrt(x**2 + y**2) for particle to have the same ID between images
...
for image in calib_col.images.values():
    name_to_z.update(
        {image.filename: float(image.filename.split(CALIB_IMG_STRING)[-1].split('.')[0]) / N_CAL})  # 'calib_X.tif' to z = X
...
if SAVE_CALIB_PLOTS or SHOW_CALIB_PLOTS:
    ...
    plot_calib_col_imgs = ['calib_4.tif', 'calib_6.tif', 'calib_8.tif', 'calib_10.tif', 'calib_12.tif', 'calib_14.tif',
                           'calib_16.tif', 'calib_18.tif']
...
for stack_id in range(len(calib_set)):
    SAVE_TEST_ID = 'gdpyt_meta_inference_stack{}'.format(stack_id)

    # Infer position using Barnkob's ('bccorr'), znccorr' (zero-normalized cross-corr.) or 'ncorr' (normalized cross-corr.)
    test_col.infer_z(calib_set, infer_sub_image=SUB_IMAGE_INTERPOLATION).bccorr(min_cm=MIN_CM, use_stack=stack_id)
