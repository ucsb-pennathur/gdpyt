...
# define filepaths
CALIB_PATH = '/Users/mackenzie/Desktop/gdpyt-characterization/calibration/particle4_2.1umNR_HighInt/calib_fc2'
CALIB_RESULTS_PATH = '/Users/mackenzie/Desktop/gdpyt-characterization/calibration/particle4_2.1umNR_HighInt/results'

EXPORT_RESULTS_PATH = '/Users/mackenzie/Desktop/gdpyt-characterization/calibration/particle4_2.1umNR_HighInt/results'
...
TRUE_NUM_PARTICLES_PER_IMAGE = 16
...
MAX_P_AREA = 550  # maximum particle size (area: units are in pixels) (recommended: 200)
SAME_ID_THRESH = 7  # maximum distance=sqrt(x**2 + y**2) for particle to have the same ID between images
...
CALIB_ID = 'CalibHalf-2.1umNileRed-'
SAVE_CALIB_ID = 'CalibHalf-2.1umNileRed-'
...
for image in calib_col.images.values():
    name_to_z.update(
        {image.filename: (float(
            image.filename.split(CALIB_IMG_STRING)[-1].split('.')[0]) - 10) / N_CAL})  # 'calib_X.tif' to z = X

# create the calibration set consisting of calibration stacks for each particle
calib_set = calib_col.create_calibration(name_to_z, dilate=True, min_num_layers=20)  # Dilate: dilate images to slide template
...
if SAVE_CALIB_PLOTS or SHOW_CALIB_PLOTS:
    ...
    plot_calib_col_imgs = [12, 14, 16, 18, 26, 29, 32, 35]
    ...
    fig = calib_set.plot_stacks_self_similarity(min_num_layers=15)
    plt.suptitle(SAVE_CALIB_ID)
    plt.tight_layout()
    if SAVE_CALIB_PLOTS is True:
...
SAVE_TEST_ID = 'calibHalf_25img_test_10img-'
...
if SAVE_PLOTS or SHOW_PLOTS:
    ...
    for IMG_INSPECT in [10, 13]:  # np.arange(1, 2, dtype=int):
        fig = test_col.plot_similarity_curve(sub_image=SUB_IMAGE_INTERPOLATION, method=INFER_METHODS, min_cm=MIN_CM,
    ...
    """
    test_col.plot_every_image_particle_stack_similarity(calib_set=calib_set, plot=False, min_cm=MIN_CM,
...
export_data = {
    ...
    'n_cal': N_CAL,
    'n_cal_averaged_per_z': 25,
    'filter': 'median disk size 3',
    'threshold': 'otsu',
