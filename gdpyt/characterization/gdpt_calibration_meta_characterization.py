...
for nl in NOISE_LEVELS:
    ...
    TEST_ID = 'Calibration-noise-level' + nl
    SAVE_TEST_ID = 'Calib-noise-level-' + CALIB_IDD + '-Test-noise-level-' + nl

    if META is True:
        test_col = calib_col # test images on identical calibration set (meta characterization)
        TEST_IMG_PATH = CALIB_IMG_PATH
    else:
        TEST_IMG_PATH = join(CALIB_PATH, TEST_ID, 'Calib-0050') # create a new image collection
    ...
    if SAVE_PLOTS is True or SHOW_PLOTS is True:
        ...
        # plot calibration images with identified particles
        plot_test_col_imgs = ['B00005.tif', 'B00010.tif', 'B00015.tif', 'B00020.tif', 'B00030.tif', 'B00035.tif', 'B00040.tif', 'B00045.tif']
        fig, ax = plt.subplots(ncols=8, figsize=(16, 4))
        ...
        for IMG_INSPECT in np.arange(4, N_CAL-4, dtype=int):
            ...
            if fig is not None:
                fig.suptitle(SAVE_TEST_ID + ': ' + r'image #/$N_{cal}$' + ' = {}'.format(np.round(IMG_INSPECT/N_CAL, 2)))
                plt.tight_layout()
                if SAVE_PLOTS is True:
                    savefigpath = join(CALIB_RESULTS_PATH, SAVE_TEST_ID + '_correlation_particles_in_img_{}.png'.format(IMG_INSPECT))
