...
# 3.1 - data analysis
thresh = 15         # particle capture radius used for sorting general dataframes to particle specific dataframe
...
for p in range(mean_locs.index.size):
    ...
    # mean_locs.values[p] = [746.2        377.17753425] --> particle coordinates
    ...
    df = pd.read_csv(filename, header=0, dtype={'E': int, 'x': float, 'y': float, 'f_e': float, 'p_index': int,
    ...
    df_filtered = df_filtered.sort_values(by=['E', 'p_index'])

    # 5.2.4.5 - perform groupby and take mean of all particles within threshold region
    #df_plot = df_filtered.groupby(['E', 'p_index']).mean()
    #print(df_plot)

    # determine unique values in E and p_ind
    range_E = df_filtered.E.unique()
    range_p_index = df_filtered.p_index.unique()


    # loop through E values
    i=0
    for e in range_E:
        data = pd.DataFrame(columns=['E', 'x', 'y', 'f_e', 'p_index', 'p_t', 'p_w', 'p_z', 'p_h'])
        lbl = str(e)

        # loop through p_index values
        for pind in range_p_index:
            df_plot = df_filtered[(df_filtered.E == e) & (df_filtered.p_index == pind)]
            # z-height min of peaks
            #plt.scatter(df_plot.p_index, df_plot.p_z, color=plt.cm.viridis(i/len(range_E)))
            # peak height
            plt.scatter(df_plot.p_index, df_plot.p_h, color=plt.cm.viridis(i / len(range_E)))
            data = data.append(df_plot,ignore_index=True)

            """
            df_mean = df_plot.groupby(['E','p_index']).mean()
            df_std = df_plot.groupby(['E', 'p_index']).std()
            plt.errorbar(df_plot.p_index, df_mean.p_z, yerr=df_std.p_z,
                     fmt='o', ecolor='g', capthick=2, solid_capstyle='projecting', capsize=5)
            """

        plt.plot(data.p_index, data.p_h, color=plt.cm.viridis(i/len(range_E)),label=lbl)
        i=i+1

    plt.title('Peak Height: X' + str(np.round(mean_locs.values[p][0], 1)) + '_Y' + str(np.round(mean_locs.values[p][1], 1)))
