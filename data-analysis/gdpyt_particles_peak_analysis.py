...
# -----------------           ----------------            ---------------------
# 4.4 - DEFINE PEAK FINDER SCRIPT

def peak_finder(df, f_img, f_e, lbl):

    # estimate deflection period
                            # f_img = 3 Hz # of images / second
                            # f_e = 0.1 Hz # of electric field cycles / second
    f = f_img / f_e         # f = 30       # of frames per electric field cycle

    # sort
    #df = df.sort_values(by=['id', 'Frame'])
    df = df.sort_index()

    # make numpy for easier manipulation
    dft = df.t.to_numpy(dtype=float)
    dfz = df.z.to_numpy(dtype=float)

    # find peaks and prominences
    # ----------------------
    peaks, properties = find_peaks(dfz,
                                   prominence=0.95,
                                   height=dfz.mean(),
                                   width=[f*.25,f*0.75],
                                   rel_height = .632)

    prominences, left_bases, right_bases = peak_prominences(dfz, peaks, wlen=f*1.1)

    prominence_data = [prominences, left_bases, right_bases]

    peak_wids = peak_widths(dfz, peaks, rel_height=0.95, prominence_data=prominence_data)

    peak_heights = dfz[peaks] - peak_wids[1]
    ...
    # peaks
    plt.plot(dft[peaks], dfz[peaks], "x")

    # peaks #1
    plt.vlines(x=dft[peaks], ymin=dfz[peaks] - properties["prominences"],
               ymax=dfz[peaks], color='lightgray', alpha=0.5)
    for i in range(len(peaks)):
        plt.hlines(y=properties["width_heights"][i], xmin=dft[int(properties["left_ips"][i])],
                   xmax=dft[int(properties["right_ips"][i])], color='lightgray', alpha=0.5)

    for i in range(len(peak_wids[0])):
        #print("peak width: " + str(peak_wids[0][i]))
        plt.hlines(y=peak_wids[1][i], xmin=dft[int(peak_wids[2][i])],
                   xmax=dft[int(peak_wids[3][i])], color='lightgray', alpha=0.5)
    ...
    for i in range(len(peaks)):
        ...
        print("y: " + str(df.y.mean()))
        print("E: " + str(lbl) + ' V/mm')
        print("f img: " + str(f_img) + ' Hz')
        print("f e: " + str(f_e) + ' Hz')
        print("peak " + str(i) + " index: " + str(peaks[i]))
        print("peak " + str(i) + " time: " + str(dft[peaks[i]]))
        print("peak " + str(i) + " width: " + str(peak_wids[0][i]))
        print("peak " + str(i) + " trough height: " + str(peak_wids[1][i]))
        print("peak " + str(i) + " height: " + str(dfz[peaks[i]]))
        """
        df_peakss.append(pd.DataFrame({
            'E': lbl,
            'x': df.x.mean(),
            'y': df.y.mean(),
            'f_e': f_e,
    ...
    if len(df_peakss) == 0:
        print("why?")
        df_peakss = pd.DataFrame()
    else:
        df_peakss = pd.concat(df_peakss).sort_index()
...
# 3.1 - data analysis
thresh = 20         # particle capture radius used for sorting general dataframes to particle specific dataframe
flip_z = 1         # invert z-direction
z_offset = 0      # offset to adjust z-position so z=0 is zero deflection plane (z_offset is subtracted from df.z)
...
# 3.5 - CREATE EMPTY DATAFRAME FOR PEAK ANALYSIS
columns = ['E', 'x', 'y', 'f_e', 'p_index', 'p_t', 'p_w', 'p_z', 'p_h']
df_peak = pd.DataFrame(data=None, columns=columns)
...
for p in range(mean_locs.index.size):
    ...
    for t in tests:
        ...
        df_plot = df_filtered.groupby(['Frame']).mean()
        ...
        if df_plot.shape[0] > frame_ratio:
            ...
            df_peaks = peak_finder(df_plot, f, f_keithley, lbl)
            df_peak = df_peak.append(df_peaks)
            ...
            # Data analysis can be performed here on
    ...
    # save csv
    df_peak.to_csv('/Users/mackenzie/Downloads/testing_09.18.20_analysis_10.08.20_BX41_IC10_870nmFP/results/results_11.3.20/peak_analysis.csv', index=False)
